...
- `package` 默认是 `ALL`，下拉只保留独立软件包 `nginx`，以及 `luci-app-aria2`、`luci-app-frpc`、`luci-app-frps`、`luci-app-gecoosac`、`luci-app-openlist2`、`luci-theme-aurora`、`luci-app-djonehub`、`luci-app-vohive` 这些 LuCI 入口；选择 LuCI 软件包时会同时编译并发布对应基础包，其中 `luci-app-aria2` 会一并处理 `aria2` 和 `ariang`，`luci-app-djonehub` / `luci-app-vohive` 会同时编译并发布对应的 `*-core` 引擎包。旧的 `aria2`、`ariang`、`frp`、`gecoosac`、`openlist2` 输入仅作为兼容别名保留。
- 想要 **`.apk` 格式**（OpenWrt 25.x 默认包管理器）请选择 `sdk_version` = **`25.12`**；`24.10` 及更早版本产出的是 `.ipk`。`*-core` 引擎包按目标架构只启用对应变体：`x86/64` → `*-core-amd64`，`aarch64` → `*-core-arm64`。
...
- 编译的软件包来源固定为：
  ...
  - `https://github.com/563617356/luci-app-djonehub` 的 `main` 分支：`djonehub-core`（含 `arm64`/`amd64`/`armv7` 变体）、`luci-app-djonehub`
  - `https://github.com/563617356/luci-app-vohive` 的 `main` 分支：`vohive-core`（含 `arm64`/`amd64`/`armv7` 变体）、`luci-app-vohive`
